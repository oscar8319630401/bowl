...
/** 플래시카드/갤러리 썸네일용 2D 실루엣 (3D를 여러 개 띄우면 무거워서) */
export function silhouetteDataUrl(
  ...
  // 3D와 같은 곡선 보간을 써야 썸네일과 실물 형태가 어긋나지 않는다
  const curve = new THREE.CatmullRomCurve3(
    profile.map((p) => new THREE.Vector3(p.r, p.y, 0)),
    false,
    'catmullrom',
    0.5,
  )
  const pts = curve.getPoints(80).map((v) => ({ r: Math.max(v.x, 0), y: v.y }))

  const maxR = Math.max(...pts.map((p) => p.r))
  const maxY = pts[pts.length - 1].y
  const scale = Math.min((size * 0.42) / maxR, (size * 0.82) / maxY)
  const cx = size / 2
  ...
  for (let i = pts.length - 1; i >= 0; i--) {
    const p = pts[i]
    ctx.lineTo(cx + p.r * scale, baseY - p.y * scale)
  }
  ctx.closePath()

  const g = ctx.createLinearGradient(cx - maxR * scale, 0, cx + maxR * scale, 0)
  const base = GLAZE[glaze].base
  g.addColorStop(0, 'rgba(0,0,0,0.45)')
  g.addColorStop(0.35, base)
  g.addColorStop(0.62, '#ffffff22')
